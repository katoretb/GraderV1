...
def main():
    try:
        ...
        if(len(cur.fetchall()) != 0):
            return jsonify({
                'success': False,
                'msg': "Student already in this class.",
                'data': {}
            })
        ...
        AddUserGrader(conn, cur, UID, UID + f"@{'student.' if UID.isnumeric() else ''}chula.ac.th", Name)
        cur.execute("INSERT INTO student (CID, UID, CSYID, GID) VALUES (%s, %s, %s, %s)", (CID, UID, CSYID, GID))
        conn.commit()
